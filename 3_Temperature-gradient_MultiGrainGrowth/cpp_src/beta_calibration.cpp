...
VelocityResult calculate_velocity_robust(const std::vector<double>& times, 
                                         ...
                                         int min_points = 5) {
    size_t n = times.size();
    // Need at least min_points + 1 (to afford skipping the first one)
    if (n < (size_t)min_points + 1) {
        // Fallback: if we have few points, try to use what we have, skipping 0 if possible
        if (n <= 1) return {0.0, 0.0, 0.0};
    }

    // Determine start index. User wants to skip the very first point (index 0).
    size_t start_idx = 1; 
    
    // We want to use a window of points.
    // Let's use up to last 20 points, but always respected start_idx.
    size_t max_window = 20;
    size_t available = n - start_idx;
    
    if (available < 2) {
        // Not enough points after skipping first. Use all if desperate?
        // Or just fail. Let's try to use index 0 if absolutely needed but prefer not to.
        if (n >= 2) start_idx = 0; 
        else return {0.0, 0.0, 0.0};
        available = n - start_idx;
    }

    size_t window_size = (available < max_window) ? available : max_window;
    // Ensure we take the *last* 'window_size' points derived from the available set
    // The range is [n - window_size, n).
    // Check if (n - window_size) < start_idx. 
    size_t range_start = n - window_size;
    if (range_start < start_idx) range_start = start_idx;
    
    // Recalculate true window size
    window_size = n - range_start;

    double sum_t = 0.0, sum_x = 0.0, sum_tx = 0.0, sum_tt = 0.0;
    double mean_x = 0.0;

    for (size_t i = range_start; i < n; ++i) {
        double t = times[i];
