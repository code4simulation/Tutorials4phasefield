...
def run_unit_test_collision(config_override=None):
    """
    C++ 커널을 사용하여 1D 상에서 두 Grain의 충돌을 테스트합니다.
    Overlap 여부를 정량적으로 검증합니다.
    """
    print("\n[UNIT TEST] Starting 1D Two-Grain Collision Test...")
    ...
    test_config = {
        ...
        'dx': 1.0, 'dt': 0.05,
        'xi': 5.0, 
        'epsilon_penalty': 20.0, # 테스트할 페널티 강도
    ...
    # Lookup Table (단일 값으로 채움)
    df_beta = pd.DataFrame({'Temperature': [500, 1500], 'Beta': [1.0, 1.0]}) 
    # dG가 양수여야 성장해서 만남
    dG_val = 2.0 # 적당한 구동력
    df_dG = pd.DataFrame({'Temperature': [500, 1500], 'dG_sim': [dG_val, dG_val]})
    
    # 핵생성 이벤트 없음 (이미 배치함)
    df_nucl = pd.DataFrame(columns=['time', 'x', 'y', 'id']) # 빈 DF
    ...
    if not os.path.exists(mpf_lib_path):
        print(f"[ERROR] Compiled library not found at: {mpf_lib_path}")
        print("Please run the main simulation once to compile the C++ code first.")
        return None

    mpf = MultiGrainWrapper(mpf_lib_path)
    # df_nucl 포맷 맞추기 (빈 데이터라도)
    if df_nucl.empty:
        # C++로 넘길 땐 최소한의 더미 데이터가 필요할 수 있음. 
        # MultiGrainWrapper 내부 로직에 따라 다르지만, 보통 빈 배열 넘겨도 됨.
        pass

    # 임시 nucleation df 생성 (에러 방지용)
    df_nucl_dummy = pd.DataFrame({
    'time': [99999.9], 
    'x_idx': [0], 
    'y_idx': [0], 
    'id': [0]
    ...
    mpf.run(phi_grid, T_profile, df_beta, df_dG, df_nucl_dummy, test_config, TEST_DIR)
    ...
    # Time-lapse 그래프 꾸미기
    plt.title(f"Collision Process (xi={test_config['xi']}, eps={test_config['epsilon_penalty']})")
    ...
    plot_filename = f"profile_xi{test_config['xi']}_eps{test_config['epsilon_penalty']}.png"
...
def optimize_penalty():
    epsilon_values = [5.0, 10.0, 20.0, 30.0]
    xi_values = [4.0, 6.0, 8.0]
    
    for xi in xi_values:
        for eps in epsilon_values:
            print(f"Testing xi={xi}, eps={eps}...")
            run_unit_test_collision({'xi': xi, 'epsilon_penalty': eps})
